########### Mexico National Inventory census 2 ###################
...
#### Read files ####
mexico_c2 <- read.csv("INFyS_Arbolado_2009_2014.csv", h = T)
mexico_c2_fire<-read.csv('incendios_census2.csv', h = T)
mexico_c2_impactos<-read.csv('impacto_census2.csv', h = T)
mexico_c2_metadata <- read.csv('metadata_census2.csv', h = T)
#mexico_c3 <- read.csv("INFyS_c3_2015_2020.csv", h = T)
#source_veg_class_table<-read.csv("source_veg_class.csv", h = T) 
disturbances_c2<-read.csv("Disturbances_census1.csv", h = T)

### Select relevant columns ### 
mexico_census2 <- mexico_c2 %>% 
  select( 
    Conglomerado:Estado,
    Formato:Tipo_veg_SV,
    Sitio,
  ...
  rename(
    stand_age = Edad,
    Height1 = Altura_total,
    condition1 = Condicion,
    long = X,
    lat = Y,
    tag_number = Arbol
  ) %>% 
  select(-Diametro_normal)

#### standardizing species names ###########
std_species <- mexico_census2 %>%
...
# Calculate AGB using chave et al allometric equations

std_species$agb_tree_kg<- round(as.numeric( 
  EQ_016( 
    std_species$DBH_original,
...
mexico_census2_tree<-std_species %>% 
...
mexico_census2_fire <- mexico_c2_fire %>% 
  select(Conglomerado, 
         Anio, 
         Estado, 
         Anio_Incendio,
...
mexico_census2_impactos<- mexico_c2_impactos %>% 
  mutate(
    std_Estado = Estado %>% stri_trans_general("Latin-ASCII") %>%  str_replace_all(" ", "_") %>% tolower,
    source_code = paste(Conglomerado, state_id$Cve_Estado_C3 [match (std_Estado, state_id$std_State_name)],sep = "_")) %>% 
  full_join(
    mexico_census2_fire,by="source_code"
  ) 

mexico_census2_meta <- mexico_c2_metadata %>% 
  mutate(
    std_Estado = Estado %>% stri_trans_general("Latin-ASCII") %>%  str_replace_all(" ", "_") %>% tolower,
    source_code = paste(Conglomerado, state_id$Cve_Estado_C3 [match (std_Estado, state_id$std_State_name)],sep = "_"),
  ...
  select(Conglomerado:Formato,
         Monitoreo:Tenencia,#Tipo_veg_SV,
         X,
         Y,
         source_code
  ) %>% 
  full_join(mexico_census2_impactos,by="source_code")


mexico_census2_metadata <- mexico_census2_meta %>%
  mutate(
    source_veg_fire = ifelse (Causa == 'Incendios', paste(Causa, TipoIncendio, sep="_"),Causa),
    Causa_EN = disturbances_c2$Distubances_en[match(source_veg_fire,disturbances_c2$Impactos_ambientales)]
  ) %>% 
  group_by(source_code, Tenencia,Anio_Incendio,Anio) %>% 
  summarise(
# Concatenate unique Causa values with a "/" separator
    source_vegetation_status_ES = paste(unique(Causa), collapse = "/"),
    source_vegetation_status = paste (unique(Causa_EN), collapse = "/") ,
# Only return "TipoIncendio" if it's not 'NA', otherwise return NA
#source_vegetation_status2 = ifelse(all(TipoIncendio == 'NA', na.rm = TRUE), NA, 
 #                                  paste(unique(TipoIncendio[TipoIncendio != 'NA']), collapse = "/")),
...
mexico_c2 <- mexico_census2_tree %>% 
  ...
  group_by(source_code, AGB_eq1, census_date, Tipo_veg_SV, std_Estado) %>% 
  summarise(
    subplot_count = n_distinct(subplot1),
    agb_plot = round(sum(agb_tree_kg, na.rm = TRUE) / 1000, 2),
    stand_age = mean (std_age, na.rm = TRUE),
    Height = mean (Height1, na.rm = TRUE),
    .groups = "drop" 
  ) %>% 
  mutate(
...
mexico_2_coordinates <- mexico_census2_tree %>%
  group_by(source_code) %>%
  filter(subplot1 == min(subplot1, na.rm = TRUE)) %>%
  slice(1) %>%  # Ensures one row per source_code in case of ties
  ungroup() %>%
...
to_view <- mexico_c2 %>% 
  left_join(mexico_census2_metadata,by="source_code") %>% 
  group_by(
    source_code,
    source_veg_class,
    census_date,
...
to_view_2 <- to_view %>% 
  left_join(mexico_2_coordinates, by = "source_code") %>% 
  mutate(
    source_vegetation_class = source_veg_class_table$Forest.Type..English.[match(to_view$source_veg_class, source_veg_class_table$std_source_veg_class)],
    source_vegetation_status_year = ifelse(
      Anio_Incendio == "Año actual", paste("fire",Anio,sep="_"),
      ifelse(Anio_Incendio == "Años anteriores", "fire_previous years", Anio_Incendio)),
    #source_vegetation_status = disturbances_c1$English.Translation[match(to_view$source_vegetation_status3, disturbances_c1$Spanish.Term)],
    #ImpactoVegetacion = ifelse(ImpactoVegetacion == "Mayor", "major", 
    #        ifelse(ImpactoVegetacion == "Mediana", "medium", 
    #              ifelse(ImpactoVegetacion == "Menor", "minor", NA))),
    #source_vegetation_status = disturbances_c1$English.Translation[match(to_view$source_vegetation_status, disturbances_c1$Spanish.Term)],
    land_ownership = land_owner$land_owner_EN[match(to_view$Tenencia, land_owner$land_owner_ES)],
    Cve_Estado_C3 = state_id$Cve_Estado_C3[match(to_view$std_Estado, state_id$std_State_name)]
  ) %>% 
  select(-c(Anio_Incendio,Anio,Tipo_veg_SV))
...
results <- numeric(length(to_view_2$Cve_Estado_C3))

for (i in 1:length(to_view_2$Cve_Estado_C3)) {
  if (is.na(to_view_2$Cve_Estado_C3[i])) {
    results[i] <- NA  # Assign NA if the value is missing
  } else if (to_view_2$Cve_Estado_C3[i] == 1) {
    results[i] <- paste("COL", as.numeric(212), sep = "")
  } else if (to_view_2$Cve_Estado_C3[i] >= 2 & to_view_2$Cve_Estado_C3[i] < 33) {
    # Calculate the additional value based on the index (statecd - 1)
    additional_value <- paste("COL", (213 + (to_view_2$Cve_Estado_C3[i] - 1)), sep = "")
...
to_view_2$collection_code<-as.character(results)  
mexico_census2_plot<-to_view_2 %>%  
...
#    plot_code=paste(collection_code,plot_code_temp,sep="_"),
    source_code=paste("INFYS",source_code,sep="_")
  ) %>% 
...
## create final output ####
write.csv(mexico_census2_plot,"Mexico_Census2_PLOT.csv",row.names=F)
