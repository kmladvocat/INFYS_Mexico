...
mexico_census3 <- mexico_c3 %>% 
 ...
 mutate(std_Estado = Estado_C3 %>% stri_trans_general("Latin-ASCII") %>%  str_replace_all(" ", "_") %>% tolower,
        source_code = paste(IdConglomerado,state_id$Cve_Estado_C3 [match (std_Estado, state_id$std_State_name)], sep = "_"),
        Height1 = ifelse(AlturaTotal_C3>100, NA, AlturaTotal_C3),
        stand_age = ifelse (Edad_C3 > 9000, NA, Edad_C3)
        ) %>%                              
...
mexico_census3$agb2_tree_kg<-(ifelse(mexico_census3$DiametroNormal_C3 > 1000 | mexico_census3$AlturaTotal_C3 > 100,
                                     NA,
                                     as.numeric(ifelse(mexico_census3$biomasa_kg_C3 == "NULL", 
                                                       NA, 
                                                       mexico_census3$biomasa_kg_C3)
                                                )))
