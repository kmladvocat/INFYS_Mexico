...
source_vegetation_table_climate<-read.csv('source_veg_table.csv', h = T) %>% 
  select (climate, source_veg_class_std_ES)
...
merging_census1_2 <- mexico_census1_plot %>% 
  full_join(mexico_census2_plot, by = "source_code") %>%
...
replace <- c(c1 = "1", c2 = "2", c3 = "3")

long_data_final <- pivot_data_mexico %>%
  mutate(
    source_vegetation_class = coalesce(
      source_veg_class_table$source_veg_class_EN[match(source_vegetation_class_ES, source_veg_class_table$source_veg_class_std_ES)],
      source_vegetation_class_ES),
    which_census = replace[which_census]
  ) %>% 
  select(-c(Cve_Estado_C3.c3,subplot_count)) %>% 
  left_join(metadata_joined, by="source_code")
